...
if(!file.exists(save_fit_fn)){
  ...
  fit <- glm_gp(sce, design = ~ celltype2 + Age + Sex + PMI + RIN + Region + 
                  DSM.IV.OUD + DSM.IV.OUD:celltype2 - 1,
...
res <- lapply(celltypes, function(cell) 
  test_de(fit, contrast = `DSM.IV.OUDOUD`, subset_to = celltype2 == cell,
          full_design = ~ DSM.IV.OUD + Age + Sex + PMI + RIN + Region + log10(nCount_RNA) - 1,
