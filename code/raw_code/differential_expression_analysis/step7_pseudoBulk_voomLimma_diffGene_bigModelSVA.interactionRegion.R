...
con_celltypes = sapply(setNames(celltypes, celltypes),function(cell) {
  ...
  OUDRegionPutamen = paste0('RegionPutamen.celltype_dxOUD.',cell) %>% make.names()
  CTLRegionPutamen = paste0('RegionPutamen.celltype_dxCTL.',cell) %>% make.names()

  paste('(',OUDRegionPutamen, '-',OUD, ')-(',CTLRegionPutamen, '-',CTL,')')
})

## the first cell type x dx is the reference cell type
con_celltypes[1] = "( RegionPutamen.celltype_dxOUD.Astrocytes - celltype_dxOUD.Astrocytes )" 
...
save_res_fn = here(rdasDir, 'OUD_Striatum_voom_limma_bigModelSVA_N22.regionInteraction.rds')
saveRDS(deg_list, save_res_fn)

tablesDir =file.path(DATADIR, 'tables'); dir.create(tablesDir, showWarnings = F)
save_res_fn2 = here(tablesDir, 'OUD_Striatum_voom_limma_bigModelSVA_N22.regionInteraction.xlsx')
...
save_res_fn3 = here(tablesDir, 'OUD_Striatum_voom_limma_bigModelSVA_N22.regionInteraction.lowConfCutOff.upReg.xlsx')
deg_list %>% lapply(function(x) x %>% arrange(P.Value) %>% filter(P.Value < 0.01, logFC > 0)) %>% 
  writexl::write_xlsx(save_res_fn3)

save_res_fn4 = here(tablesDir, 'OUD_Striatum_voom_limma_bigModelSVA_N22.regionInteraction.lowConfCutOff.dnReg.xlsx')
deg_list %>% lapply(function(x) x %>% arrange(P.Value) %>% filter(P.Value < 0.01, logFC < 0)) %>% 
