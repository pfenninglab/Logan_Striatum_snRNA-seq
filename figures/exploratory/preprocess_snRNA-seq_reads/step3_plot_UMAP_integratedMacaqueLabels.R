...
DimPlot(object = obj_merged, reduction = "umap", 
...
DimPlot(object = obj_msn, reduction = "umap", group.by = 'orig.ident', 
...
DimPlot(object = obj_msn, reduction = "umap",  group.by = 'seurat_clusters', split.by = 'orig.ident') +
  guides(color = guide_legend(nrow = 1, override.aes= list(size = 3))) + 
  theme(legend.position = 'bottom')
dev.off()
...
DimPlot(object = obj_msn, reduction = "umap", split.by = 'orig.ident', 
        group.by = 'celltype2',  cols = subtypes_col) +
