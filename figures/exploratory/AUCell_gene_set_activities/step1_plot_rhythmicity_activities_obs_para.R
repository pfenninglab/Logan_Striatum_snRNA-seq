...
cells_AUC_dflong = cells_AUC_df %>% 
  ...
  group_by(Case, Sex, DSM.IV.OUD, celltype3, GeneSet) %>% 
  summarize(AUC= mean(AUC)) %>% ungroup()
  
###################################################
## 2) plot the differences in the AUCell gene sets
pdf(here(PLOTDIR, 'AUCell_Xue_rhythmicity_scores_byCelltype3.NAc.pdf'), width = 6, height = 2.5)
...
pdf(here(PLOTDIR, 'AUCell_Xue_rhythmicity_scores_byCelltype3.DLPFC.pdf'), width = 6, height = 2.5)
