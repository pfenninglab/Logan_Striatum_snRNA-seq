...
glia_types = c('Astrocytes','Endothelial', 'Microglia', 'Oligos', 'Oligos_Pre')
...
res = readRDS(save_res_fn)[c('Glia', 'All', glia_types)]
...
stats_group  = here('figures/exploratory/AUCell_gene_set_activities', 'tables', 'OUD_Striatum_pyscenic_differential_TF_modules.byCelltype.xlsx') %>% 
  readxl::read_xlsx() %>% filter(celltype3 %in% glia_types ) %>% 
...
select_TF = c("NFKB1", 'FOXP2', 'RXRG', 'CEBPB')
...
for(plot_TF in select_TF){
  ...
  plot_fn = here(PLOTDIR,'plots', paste0('figure2_TF-gene_modules_',plot_TF,'.pdf'))
