...
stats_group  = here('figures/exploratory/AUCell_gene_set_activities', 'tables', 'OUD_Striatum_pyscenic_differential_TF_modules.byCelltype.xlsx') %>% 
  readxl::read_xlsx() %>% filter(grepl('^D|Int', celltype3) ) %>% 
...
plot_tf_enrichment_fn = here(PLOTDIR, 'plots', 'figure3_TF_regulon_neuron.heatmap.pdf')
pdf(plot_tf_enrichment_fn, height = 80/in2mm, width = 59/in2mm)
